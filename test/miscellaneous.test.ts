import { ChainId, InsufficientInputAmountError, Pair, ProtocolName, Token, TokenAmount } from '../src'
import { sortedInsert } from '../src/utils'

describe('miscellaneous', () => {
  it('getLiquidityMinted:0', async () => {
    const tokenA = new Token(ChainId.MTV_MAINNET, '0x0000000000000000000000000000000000000001', 18)
    const tokenB = new Token(ChainId.MTV_MAINNET, '0x0000000000000000000000000000000000000002', 18)
    const pair = new Pair(new TokenAmount(tokenA, '0'), new TokenAmount(tokenB, '0'), ProtocolName.AMATERASU)
  ...
  it('getLiquidityMinted:!0', async () => {
    const tokenA = new Token(ChainId.MTV_MAINNET, '0x0000000000000000000000000000000000000001', 18)
    const tokenB = new Token(ChainId.MTV_MAINNET, '0x0000000000000000000000000000000000000002', 18)
    const pair = new Pair(new TokenAmount(tokenA, '10000'), new TokenAmount(tokenB, '10000'), ProtocolName.AMATERASU)
  ...
  it('getLiquidityValue:!feeOn', async () => {
    const tokenA = new Token(ChainId.MTV_MAINNET, '0x0000000000000000000000000000000000000001', 18)
    const tokenB = new Token(ChainId.MTV_MAINNET, '0x0000000000000000000000000000000000000002', 18)
    const pair = new Pair(new TokenAmount(tokenA, '1000'), new TokenAmount(tokenB, '1000'), ProtocolName.AMATERASU)
